...
const Product = () => {
  ...
  const { products, formatCurrency, addToCart, t } = useContext(ShopContext);
    ...
    <div  className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      ...
      <div className='flex-1'>
        ...
        <div className='flex flex-col gap-4 my-8'>
          ...
          <button onClick={() => addToCart(productData._id,size)} className='bg-black text-white px-8 py-3 text-sm active:bg-gray-700'>{t('product.addToCart')}</button>
        <hr className='mt-8 sm:w-4/5'/>
        <div className='text-sm text-gray-500 mt-5 flex flex-col gap-1'>
            <p>{t('product.original')}</p>
      ...
      <div className='mt-20' >
        <div className='flex'>
          <b className='border px-5 py-3  text-sm'>{t('product.descriptionTab')}</b>
          <p className='border px-5 py-3 text-sm'>{t('product.reviewsTab')}</p>
        </div>
